module Gen.Route.Support exposing (annotation_, make_, moduleName_, route, values_)
...
annotation_ =
    ...
    , msg = Type.namedWith [ "Route", "Support" ] "Msg" []
    }
...
make_ =
    ...
    , data =
        \data_args ->
            Elm.withType
                (Type.alias [ "Route", "Support" ] "Data" [] (Type.record []))
                (Elm.record [])
    , routeParams =
        \routeParams_args ->
            Elm.withType
